...
def publish(
    ...
    out: Path = typer.Option(..., "--out", help="Output bundle root directory"),
) -> None:
...
@app.command("upload")
def upload(bundle: Path = typer.Option(..., "--bundle")) -> None:
...
@app.command("verify")
def verify(bundle: Path = typer.Option(..., "--bundle")) -> None:
...
def schema_export(out: Path = typer.Option(..., "--out", help="Output JSON Schema file")) -> None:
